...
import com.kbds.dgdgtalk.auth.value.OAuthAttributes;
 ...
 * Description : 가져온 사용자의 정보들을 기반으로 가입 및 정보 수정, 세션 저장 등의 기능 지원
 *
 *
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // registrationId : 현재 소셜 서비스를 구분하는 코드
        String registrationId = userRequest.getClientRegistration().getRegistrationId(); 
        
        // userNameAttributeName : OAuth2로 로그인 진행 시 키가 되는 필드값, 구글은 기본적으로 지원
        String userNameAttributeName = userRequest.getClientRegistration().getProviderDetails()
                                        .getUserInfoEndpoint().getUserNameAttributeName(); 

        // OAuthAttributes : UserService를 통해 가져온 OAuth2User의 attribute를 담는 클래스
